...
export default function GroupCard({ group } : GroupCardProps){
  ...
  return (
    <div className={`group relative rounded-2xl border border-zinc-200/70 bg-white px-4 py-3 shadow-sm transition hover:-translate-y-[1px] hover:shadow-md active:scale-[0.99] ${isEnd ? "opacity-80 cursor-not-allowed" : "cursor-pointer"}`}>
      {/* Overlay */}
      {isEnd && <div className="pointer-events-none absolute inset-0 rounded-2xl bg-zinc-50/60 backdrop-blur-[1px]" />}
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {/* group badges */}
          {group.badges.map((b) => (
            <span key={b} className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${BADGE_CLASS[b]}`}>
              {GROUP_BADGE_LABEL[b]}
            </span>
          ))}
        </div>

        <span className={`shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${isEnd ? "bg-zinc-100 text-zinc-600 ring-zinc-200" : "bg-zinc-900/5 text-zinc-800 ring-zinc-200"}`}>
          {isEnd ? "마감" : "모집중"}
        </span>
      </div>
      ...
      <div className="relative flex items-center justify-between text-xs">
        <span className="inline-flex items-center gap-1 text-zinc-500">
          <Image src="/icons/users.svg" alt="Users" width={16} height={16} />
